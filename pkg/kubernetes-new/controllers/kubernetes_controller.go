...
type KubernetesController struct {
	...
	resourceCalc   *k8sLogic.ResourceCalculator
}
...
func (kc *KubernetesController) GetClusterHealth(ctx context.Context, context string) (*k8sPorts.ClusterHealth, error) {
	// Get cluster info
	...
	var nodeHealths []k8sPorts.NodeHealth
	for _, node := range clusterInfo.Nodes {
		nodeHealth := k8sPorts.NodeHealth{
			Name:        node.Name,
			Status:      node.Status,
